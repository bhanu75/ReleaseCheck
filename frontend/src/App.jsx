...
import { api } from "./api"; // Aapka banaaya hua API layer connect kiya
...
const css = {
  ...
  logo: { fontWeight: 700, fontSize: 18, color: "#4f46e5", letterSpacing: "-0.5px", cursor: "pointer" },
  ...
  loading: { textAlign: "center", padding: "3rem 0", fontSize: 16, color: "#4f46e5", fontWeight: 600 }
};
...
function NewReleaseModal({ onClose, onCreate }) {
  ...
  const submit = () => {
    ...
    onCreate({ id: genId(), name: name.trim(), date: new Date(date).toISOString(), info, completedSteps: [] });
    onClose();
  };
...
export default function App() {
  ...
  // Database se live data fetch karne ke liye
  useEffect(() => {
    api.getReleases()
      .then(data => {
        setReleases(data);
  ...
  const handleCreate = useCallback((newRelease) => {
    api.createRelease(newRelease)
      .then(savedRelease => {
        setReleases(prev => [savedRelease, ...prev]);
      })
      .catch(err => alert("Failed to create release in DB: " + err.message));
  }, []);
  ...
  const handleDelete = useCallback((id) => {
    if (confirm("Delete this release?")) {
      api.deleteRelease(id)
        ...
        .catch(err => alert("Failed to delete release from DB: " + err.message));
    }
  }, [activeId]);
  ...
  const handleToggleStep = useCallback((id, stepIndex) => {
    ...
    const has = target.completedSteps.includes(stepIndex);
    const updatedSteps = has 
      ? target.completedSteps.filter(i => i !== stepIndex) 
      : [...target.completedSteps, stepIndex];

    api.updateSteps(id, updatedSteps)
      .then(updatedRelease => {
        setReleases(prev => prev.map(r => r.id === id ? updatedRelease : r));
      })
      .catch(err => alert("Failed to update steps in DB: " + err.message));
  }, [releases]);

  const handleSaveInfo = useCallback((id, info) => {
    api.updateInfo(id, info)
      .then(updatedRelease => {
        setReleases(prev => prev.map(r => r.id === id ? updatedRelease : r));
      })
      .catch(err => alert("Failed to save notes in DB: " + err.message));
  }, []);
  ...
  return (
    <div style={css.root}>
      ...
      <main style={css.main}>
        {loading ? (
          <div style={css.loading}>Loading your checklist from database...</div>
        ) : !activeRelease ? (
